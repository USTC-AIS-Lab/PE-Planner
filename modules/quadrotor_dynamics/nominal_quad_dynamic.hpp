...
public:
    ...
    void xdot_func(const Matrix<double, x_dim_, 1> &x, const VectorXd &u, const Vector3d &disturbance_acc,
        ...
        const double &ry = u[1];
        const double &rz = u[2];
        const double thrust = /*1.084e-5 * pow(u[3] * 1000 + 100, 2) * 4 / 0.74;//*/u[3] * 9.81 / hover_ratio_;
        xdot << vx, vy, vz, 
                2 * (qx * qz + qw * qy) * thrust + disturbance_acc.x(), 2 * (qy * qz - qw * qx) * thrust + disturbance_acc.y(), ((1 - 2 * (qx * qx + qy * qy)) * thrust - 9.81 + disturbance_acc.z()),
