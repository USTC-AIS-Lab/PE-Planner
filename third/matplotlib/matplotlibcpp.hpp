...
template<typename Numeric>
bool plot(const std::vector<Numeric> &x, const std::vector<Numeric> &y, const string& s, const std::map<std::string, std::string>& keywords)
{
    assert(x.size() == y.size());
...
inline void colorbar(PyObject* mappable = NULL, const std::map<std::string, std::string>& keywords = {})
{
    // if (mappable == NULL)
